...
class AcousticOptimizer:
    def __init__(
        ...
        self.stall_count = 0
        self.min_stall_iterations = (
            40  # Require this many stalled iterations before claiming convergence
        )

        # Setup Bayesian optimization
        self.space = [
            Real(low, high, name=param) for param, (low, high) in self.bounds.items()
        ]
    ...
    def generate_exploration_points(self, n_points: int) -> List[Dict[str, float]]:
        ...
        while len(points) < n_points and attempts < max_attempts:
            ...
            candidate_points = self.optimizer.ask(
                n_points=min(n_points - len(points), self.n_parallel)
            )
    ...
    def check_convergence(self) -> bool:
        ...
        relative_improvement = (
            max_recent - self._last_iter_best_score
        ) / self._last_iter_best_score
        ...
        for iteration in range(0, n_iterations, self.n_parallel):
            ...
            with ThreadPoolExecutor(max_workers=self.n_parallel) as executor:
                ...
                for i, future in enumerate(futures):
                    ...
                    absorber_area = point["ceiling_center_width"] * (
                        point["ceiling_center_xmax"] - point["ceiling_center_xmin"]
                    )
                    ...
                    if simulation_result["status"] == "success":
                        ...
                        avg_energy_over_window = simulation_result["results"].get(
                            "avg_energy_over_window"
                        )
                    ...
                    self.optimizer.tell(
                        [point[param] for param in self.param_names], -score
                    ...
                    if score > self.best_score:
                        ...
                        self.logger.info(
                            f"\t: avg_energy_25ms: {store_result['foms']['avg_energy_over_window']}"
                        )
                        self.logger.info(
                            f"\t: absorber_area: {store_result['foms']['absorber_area']}"
                        )
    ...
    def plot_progress(self, reset_best_so_far: bool = False):
        ...
        if hasattr(self, "_plot_from"):
            plt.plot(
                iterations[self._plot_from :],
                np.maximum.accumulate(scores[self._plot_from :]),
                "r-",
                label="Best so far",
            )
        else:
            plt.plot(
                iterations, np.maximum.accumulate(scores), "r-", label="Best so far"
            )
...
if __name__ == "__main__":
    ...
    optimizer = AcousticOptimizer(
        ...
        n_parallel=16,
        bounds={
            "distance_from_front": (0.41, 0.6),
            "distance_from_center": (0.7, 1.0),
